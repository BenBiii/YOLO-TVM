...
# TVM 빌드 시 RPC 기능이 활성화되어 있지 않으면 이후 원격 실행(리모트) 단계에서 오류가 발생하므로, 사전에 체크
...
# Darknet 유틸(load_image)로 로드 후 (H, W, C) → (C, H, W)로 차원 변경
